<x-app-layout>
    <main class="h-auto 
        @if (auth()->check() && ($user->role === 'seller' || $user->role === 'admin')) mt-10 md:ml-64 @endif">
        <section class=" bg-white dark:bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern.svg')] dark:bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')]">
            <div class="p-28 py-8 px-10 mx-auto max-w-screen-xl flex flex-col lg:flex-row items-center lg:justify-between text-center lg:text-left">
                ...
                <div>
                    ...
                    @if (auth()->check() && ($user->role == 'seller' && $store->seller_id == $user->id)) 
            ...
            <div class="mb-2 grid gap-4 sm:grid-cols-2 sm:gap-8 lg:gap-16">
                <div class="space-y-4">
                    <div class="flex space-x-4">
                        <img class="h-16 w-16 rounded-lg" src="{{ Storage::url($store->Seller->profileImage) }}" alt="Helene avatar" />
                        <div class="flex items-center" >
                        <h2 class="flex items-center text-xl font-bold leading-none text-gray-900 sm:text-2xl">{{ $store->Seller->name }}</h2>
                        </div>
                    </div>
                    <dl class="">
                        <dt class="font-semibold text-gray-900">Email Address</dt>
                        <dd class="text-gray-500 dark:text-gray-400">{{ $store->Seller->email }}</dd>
                      </dl>
                </div>
